...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      setIsLoading(true);
      try {
        const [saleRes, newRes, bestSellerRes] = await Promise.all([
          getProducts({ limit: 4, sort: 'price_asc' }),
          getProducts({ limit: 4, sort: 'newest' }),
          getProducts({ limit: 4, sort: 'best_seller' })
        ]);
  ...
  return (
    <div className="w-full">
      ...
      <div className="max-w-7xl mx-auto px-4 py-24 space-y-32">
        ...
        {/* Section Wrapper */}
        {[
          { title: 'Sản Phẩm Khuyến Mãi', accent: 'bg-primary-600', products: saleProducts, color: 'text-primary-600' },
          { title: 'Sản Phẩm Mới Nhất', accent: 'bg-black', products: newProducts, color: 'text-black' },
          { title: 'Sản Phẩm Bán Chạy', accent: 'bg-orange-500', products: bestSellerProducts, color: 'text-orange-500' }
        ].map((section, idx) => (
          <section key={idx}>
